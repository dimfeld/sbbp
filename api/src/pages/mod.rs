...
async fn home_page(
    ...
    let body = html! {
        main ."relative p-4 flex flex-col gap-4" {
            form ."flex flex-col gap-2 rounded-lg border border-border p-4"
                method="POST"
                action="_action/add_video" {
                    label ."flex gap-2 flex-1 max-w-[100ch] text-base" for="path" { "Add new video "}
                    div .flex .gap-2 {
